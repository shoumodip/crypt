const field = {
  ...
  mul: (a, b) => {
    ...
    return field.exp[(field.log[a] + field.log[b]) % 255]
  },
  ...
  div: (a, b) => {
    ...
    let p = (field.log[a] - field.log[b]) % 255
    if (p < 0) {
      p += 255
    }
...
const crypt = {
  decode: (shares) => {
    if (shares.length < 2) {
      throw new Error("too few shares")
    }

    if (shares[0].length < 2) {
  ...
  encode: (secret, n, k) => {
    ...
    if (n <= k) {
      throw new Error("minimum shares must be less than total shares")
    }
    ...
    for (const x of secret) {
      ...
      poly[0] = x.charCodeAt(0)

      const buffer = crypto.getRandomValues(new Uint8Array(k - 2))
      for (let i = 1; i < k - 1; i++) {
        poly[i] = buffer[i - 1]
      }

      while (true) {
        const buffer = crypto.getRandomValues(new Uint8Array(1))
        if (buffer[0] != 0) {
          poly[k - 1] = buffer[0]
          break
        }
      ...
      for (let x = 1; x <= n; x++) {
        ...
        for (let i = 1; i <= poly.length; i++) {
          result = field.add(field.mul(result, x), poly[poly.length - i])
        }
